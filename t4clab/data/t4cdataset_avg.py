...
class T4CTrainingDataSet(Dataset):
    def __init__(self, data_dir="nfs/shared/traffic4cast", city=None, threshold=0.2, input_size=12, include_timestamps=True):
        ...
        self.average = torch.tensor(np.load(data_dir + "/" + city + "_traffic_per_weekday_masked.npy"))
    ...
    def __getitem__(self, idx):
        ...
        sample = build_sample(dynamic_data, self.edges, i, day_of_week, self.input_size, self.include_timestamps, self.average[day_of_week, i:i+24, :, :, :])
        sample["mask"] = self.mask
        sample["average"] = self.average[day_of_week, i:i+24, :, :, :]
        return sample
...
class T4CTestDataSet(Dataset):
    def __init__(self, data_dir="nfs/shared/traffic4cast", city=None, threshold=0.2, input_size=12, include_timestamps=True):
        ...
        _ , self.edges = create_edge_index(self.static_data, self.threshold)
        self.mask = self.static_data[0]>self.threshold
        self.average = torch.tensor(np.load(data_dir + "/" + city + "_traffic_per_weekday_masked.npy")).type(torch.FloatTensor)
...
class T4CDataModule(pl.LightningDataModule):
    def __init__(self, data_dir: str = "/nfs/shared/traffic4cast", batch_size: int = 1, input_size=12, city: str = "MELBOURNE", threshold=0.2, include_timestamps=True):
        super().__init__()
